...
/* USER CODE BEGIN PV */
#include "C:\Users\jiho\STM32CubeIDE\workspace_1.12.1\myLib/myLib.c"
...
void StartDefaultTask(void const * argument)
{
  ...
  /* Infinite loop */
	...
	for(;;)
	{
		...
		{
			printf("\033[7;1Hin default Task......%d   \r\n", n++);
...
void StartTask02(void const * argument)
{
  ...
  for(;;)
  {
	  if(osSemaphoreWait(myBinarySem01Handle,0) == osOK)
	  	{
		  printf("\033[5;1Hin second Task......%d    \r\n", n++);
		  HAL_GPIO_TogglePin(LDg_GPIO_Port,LDg_Pin);
		  HAL_Delay(100);
			osSemaphoreRelease(myBinarySem01Handle);
...
void StartTask03(void const * argument)
{
	...
	for(;;)
  {
		 if(osSemaphoreWait(myBinarySem01Handle,0) == osOK)
			  	{
					printf("\033[3;1Hin third Task......%d     \r\n", n++);
					HAL_GPIO_TogglePin(LDy_GPIO_Port,LDy_Pin);
					HAL_Delay(100);
					osSemaphoreRelease(myBinarySem01Handle);
